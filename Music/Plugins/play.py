...
@Client.on_message(command(["/play", "سوريا" ,"صلاح" ,"الماس" ,"شغل" ,"تشغيل", f"/play@{BOT_USERNAME}", "تشغيل"]))
async def play(_, message: Message):
    chat_id = message.chat.id
    if message.sender_chat:
        return await message.reply_text(
            """
عارف ان حلم الاعضاء يشغلو اغاني بس اسف مش هينفع 😿😂
"""
        )
...
قم بالغاء الحظر للاستخدام البوت بنجاح 🎶️
"""
            )
            return
        ...
        if message.chat.username:
            try:
                ...
                await message.reply(
                    f"{ASSNAME} تم انضمام الحساب المساعد بنجاح ي كابتن 😿 ،،",
                )
...
"""
            ...
            try:
                ...
                await message.reply(f"{ASSNAME} تم انضمام المساعد بنجاح ي كابتن . 😿",)
                await remove_active_chat(chat_id)
